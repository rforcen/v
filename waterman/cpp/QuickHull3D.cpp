...
void QuickHull3D::cleanup() {
  for (auto &face : faces) delete face;  // delete faces
  faces.clear();
  for (auto &he : horizon) delete he;  // delete horizon
  horizon.clear();
  for (auto &p : pointBuffer) delete p;  // delete points
...
void QuickHull3D::initBuffers(int nump) {
  if ((int)pointBuffer.size() < nump) {  // resize
    vertexPointIndices.resize(nump);
    while (pointBuffer.size() < nump) pointBuffer.push_back(new Vertex);
  }
...
void QuickHull3D::createInitialSimplex() {
  ...
  Vector3d u01, diff02, nrml, xprod;
  double maxSqr = 0;
...
Vertex *QuickHull3D::nextPointToAdd() {
  if (!claimed->isEmpty()) {
    ...
    Vertex *eyeVtx = null;
    double maxDist = 0;
    for (Vertex *vtx = eyeFace->outside; vtx != null && vtx->face == eyeFace;
         vtx = vtx->next) {
...
bool QuickHull3D::check(double tol) {
  ...
  if (!checkFaces(tolerance)) return false;
