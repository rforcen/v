...
import vmath, sequtils, wp, strformat
...
const
  DOUBLE_PREC = 2.2204460492503131e-16
  HIDDEN = -1
  DISABLED = 0
  VISIBLE = 1
...
proc head(he: ptr HalfEdge): ptr Vertex = he.vertex
proc tail(he: ptr HalfEdge): ptr Vertex =
...
proc lengthSq(he: ptr HalfEdge): float =
...
proc reindexFacesAndVertices(q: var QuickHull3D) =
  for pb in q.pointBuffer.mitems: pb.index = -1

  # remove inactive faces and mark active vertices
  var new_faces : FaceVector = @[]
  ...
  for face in q.faces.mitems:
    q.markFaceVertices(face, 0) # indexed from 1

  # reindex vertices
  q.numVertices = 0
  for i in 0..q.pointBuffer.high:
    if q.pointBuffer[i].index == 0:
      q.vertexPointIndices[q.numVertices] = i
      q.pointBuffer[i].index = q.numVertices
      inc q.numVertices
  echo fmt("numVertices:{q.numVertices} # points in buffer:{q.pointBuffer.len}")
...
proc getVertices(q: QuickHull3D, scale:bool=true): seq[Point3D] =
  ...
  echo fmt("nv:{q.numVertices} pb:{q.pointBuffer.len}")
...
# global interfaced proc's (returns faces, vertex)
...
when isMainModule:
  let w = waterman_poly(100.0)
  let (faces, vertices) = convexHull(w)
  echo fmt("wp # verts:{w.len} faces:{faces.len} vertices:{vertices.len}")
  # 79
  # 14
  # 23 <-> 24
